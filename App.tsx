...
const App: React.FC = () => {
  ...
  const handleDownloadPDF = async () => {
    ...
    try {
      ...
      // Calculate layout for 2x2 grid (4 cards per page)
      const GRID_COLS = 2;
      const GRID_ROWS = 2;
      ...
      for (let i = 0; i < students.length; i++) {
        ...
        // Add a new page every 4 items (2x2 grid)
        ...
        try {
          ...
          const commonOptions = {
            quality: 0.9,
            pixelRatio: 2.5, // ~300 DPI for 80mm width
            cacheBust: true,
          ...
          // Add Image
          doc.addImage(frontDataUrl, 'JPEG', x, y, CARD_WIDTH, CARD_HEIGHT);

          // Draw Crop Marks
          drawCropMarks(x, y, CARD_WIDTH, CARD_HEIGHT);

          // Label text
          doc.setFontSize(8);
          doc.setTextColor(150, 150, 150);
          doc.text(`Student: ${students[i].name} (${students[i].studentId})`, x, y - 3);
          ...
          {students.length > 0 && (
            <>
              ...
              >
                ...
                {isGenerating ? `Processing ${progress}%` : 'Download Fronts PDF'}
              </button>
              <button 
                onClick={clearData}
                disabled={isGenerating}
